...
{
    @list($jour, $mois, $annee) = explode('/', $maDate);//@list - assigne des variables comme si elles étaient en tableau 
    return date('Y-m-d', mktime(0, 0, 0, $mois, $jour, $annee));//mktime transforme le format de la date
}
...
{
    @list($annee, $mois, $jour) = explode('-', $maDate);//explode coupe une chaine en chainement
...
function getMois($date)
{
    ...
    unset($jour);//unset détruit une vriable
    if (strlen($mois) == 1) {//strlen calcule la taille d'une chaine
        $mois = '0' . $mois;
    }
    return $annee . $mois;
}
...
{
    return preg_match('/[^0-9]/', $valeur) == 0;//preg_match effectue une recherche de correspondance avec une expression rationelle strandard
}
